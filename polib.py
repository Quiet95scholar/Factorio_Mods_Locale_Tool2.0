...
class POFile(_BaseFile):
    ...
    def absorb(self, refpot):
        ...
        * the fuzzy flags are preserved.
        ...
        other_V = refpot.getv()
        if self_v < other_V:
            self.metadata['Project-Id-Version'] = other_V

        # Store entries in dict/set for faster access
        self_entries = dict((entry.msgctxt, entry) for entry in self)
        refpot_msgctxt = set(entry.msgctxt for entry in refpot)
        # Merge entries that are in the refpot
        for entry in refpot:
            e = self_entries.get(entry.msgctxt)
            if e is None:
                e = POEntry()
                e.tcomment = "从" + other_V + "版本添加"
                self.append(e)
            e.absorb(entry, self_v, other_V)
        # ok, now we must "obsolete" entries that are not in the refpot anymore
        for entry in self:
            if entry.msgctxt not in refpot_msgctxt:
                # //TODO 输出文字最好能够本地化
                if entry.comment == "":
                    entry.comment = "从" + other_V + "版本移除"
...
class POEntry(_BaseEntry):
    ...
    def absorb(self, other, self_v=0, other_v=0):
        self.msgctxt = other.msgctxt
        # 继承库中flags:fuzzy标记和被吸收项的所有flags标记
        fuzzy = 'fuzzy' in self.flags
        self.flags = other.flags[:]  # clone flags
        if fuzzy:
            self.flags.append('fuzzy')
        self.occurrences = other.occurrences
        # 如果库中有过时(此过时标记是写在commt中的)标记则保持不变
        if self.comment == "":
            self.comment = other.comment
        if self.msgid == "":
            self.msgid = other.msgid
            ...
            if self.msgid == other.msgid:
                ...
                if self_v <= other_v:
                    self.previous_msgctxt = self.msgid
                    self.previous_msgid = self.msgstr
                    self.msgid = other.msgid
                    self.msgstr = other.msgstr
        # 负数形式,暂时不用
        if other.msgstr_plural:
